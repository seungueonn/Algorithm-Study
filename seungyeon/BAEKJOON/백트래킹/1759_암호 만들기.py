...
for i in range(c):
    if arr[i] in ['a','e','i','o','u']:
        alpa.append([arr[i],True])
    else:
        alpa.append([arr[i],False])

def back(idx,answer,visited, a, b):

    if len(answer) == l and a >= 1 and b >= 2:
        print(''.join(answer))
        return 
        # return answer
    ...
    for i in range(idx,c):
        ...
        if not visited[i]:
            ...
            visited[i] = True
            answer.append(alpa[i][0])
            
            back(i+1,answer,visited,a,b)
